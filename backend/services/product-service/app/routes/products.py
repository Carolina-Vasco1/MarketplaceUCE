...
async def list_products(
    ...
    docs = await products.find(query).sort("_id", -1).limit(limit).to_list(length=limit)

    out: list[ProductOut] = []
    for d in docs:
        pid = str(d.get("_id"))
        d.pop("_id", None)
        out.append(ProductOut(id=pid, **d))
    return out
